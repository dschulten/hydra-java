...
The ˋSirenMessageConverterˋ renders Spring Hateoas Responses as https://github.com/kevinswiber/siren[Siren] messages, using the media type ˋapplication/vnd.siren+jsonˋ. It renders a plain Spring Hateoas ˋLinkˋ as embedded link or navigational link and a templated link as a Siren GET action with fields for the template query variables. In order to produce other Siren actions, point to your POST or PUT method using AffordanceBuilders linkTo-methodOn idiom, as shown above in the section AffordanceBuilder.

Possible values are treated as checkbox or radio button fields.


The corresponding Siren output looks like this:
[source, Javascript]
----
<tbd>
----
