...
information coming from outside the resource itself. That is due to the nature of json. Two solutions immediately come to mind: 
...
Or put up documentation pages to describe your extension relation types and make the documentation available by dereferencing http://example.com/api/rels#doodad.
...
one-fancy-api requires to achieve that without processing a vendor-specific documentation. How can we do that, purely based on a media type and relation names?
...
e.g. by using http://schema.org/additionalType[additionalType] to point to http://purl.org/goodrelations/[GoodRelations] or by using external enumerated values as shown by http://schema.org/DeliveryMethod[DeliveryMethod].
...
For a more expressive example consider the json-ld example of https://schema.org/MusicEvent, which shows how a ticket offering could look like.

== First Steps

It is currently possible to render a json-ld in the schema.org context from a Java bean with hydra-java. Some basic annotation-based customizations are also in place, notably:
...
Look into https://github.com/dschulten/hydra-java/blob/master/src/test/java/de/escalon/hypermedia/hydra/serialize/JacksonHydraSerializerTest.java[JacksonHydraSerializerTest] to see how it works :)
