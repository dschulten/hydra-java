...
public interface Options {
     ...
     * </pre>
     * <p/>
     * The <code>&#064;Select</code> annotation above says that the possible detail values come from a DetailOptions bean which
     * determines those values based on the personId. Note how the personId is passed to showDetails as argument
     * to the same call which also has an argument annotated with <code>@Select(options=...)</code>.
     * This allows us to resolve the <code>"personId"</code> arg defined for DetailOptions to an actual value.
     * <p/>
     * <pre>
     ...
     * 	Affordance action = linkTo(methodOn(this.getClass()).showDetails(personId, null), &quot;showDetails&quot;);
     ...
     *              values received in the same method call to an implementation of {@link Options}. See
